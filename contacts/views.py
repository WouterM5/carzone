...
from contacts.models import Contact, Form
...
def contactform(request):
    if request.method == 'POST':
        ...
        contactform = Form(first_name=first_name, last_name=last_name, email=email, phone=phone, message=message)

        admin_info = User.objects.get(is_superuser=True)
        admin_email = admin_info.email
        send_mail(
                'New Contact',
                'You have a new contact. Please login to your admin panel for more info.' + message,
                'wouterm.developer@gmail.com',
                [admin_email],
                fail_silently=False,
                    )
